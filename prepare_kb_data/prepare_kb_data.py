...
from utils import save_docs_as_json, save_docs_as_md
from scrapper import APRScrapper
...
def identify_airtravel_issue(df: pd.DataFrame) -> pd.DataFrame:
    ...
    issues_key_words = {
        "flight_delays_cancellations": ["flight-delay", "flight-cancellation"],
        "lost_damaged_delayed_baggage": ["baggage"],
        "denied_boarding": ["boarding"],
    }
...
def main():
    ...
    # remove documents with less than 40 characters
    docs_df = docs_df[docs_df.text.str.len() > 40].reset_index(drop=True)

    # identify the topic of each document
