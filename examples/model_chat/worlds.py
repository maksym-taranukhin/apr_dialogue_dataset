...
class ModelChatOnboardWorld(CrowdOnboardWorld):
    ...
    def parley(self):
        ...
        if not self.skip_onboarding:
            ...
            print(f"{self.__class__.__name__}: starting parley for worker_id: {self.worker_id}")
...
class BaseModelChatWorld(CrowdTaskWorld, ABC):
    ...
    def parley(self):
        ...
        for idx, agent in enumerate([self.agent, self.bot]):
            if not self.chat_done:
                ...
                if agent == self.bot and hasattr(self.bot, "agent_id") and self.bot.agent_id:
    ...
    def _prepare_acceptability_checking(self) -> Tuple[List[str], List[str]]:
        ...
        """
        human_messages = [message["text"] for message in self.dialog if message["agent_idx"] == 0]
...
class ModelChatWorld(BaseModelChatWorld):
    ...
    def _run_initial_turn(self) -> None:
        ...
        if self.opt["conversation_start_mode"] == "blended_skill_talk":
            ...
            raise ValueError(
                f"Conversation start mode {self.opt['conversation_start_mode']} " f"not recognized!"
            )
...
def make_world(opt, agents):
    ...
    return ModelChatWorld(opt, agent=agents[0], bot=bot_worker, context_info=context_info)
