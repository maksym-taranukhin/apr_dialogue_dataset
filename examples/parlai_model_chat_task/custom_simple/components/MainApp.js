...
function MainApp() {
  ...
  const [selectedSpans, setSelectedSpan] = React.useState([]);

  const addSelectedSpan = (span) => {
    setSelectedSpan([...selectedSpans, span]);
  }
  const removeSelectedSpan = (span) => {
    setSelectedSpan(selectedSpans.filter((s) => s !== span));
  }

  const handleAppendText = (span) => {
    setAppendText(span.selectedText);
  };
        ...
        */
        ...
        renderSidePane={({ mephistoContext: { taskConfig } }) => (
          <StatementList
            ...
            // onSpanSelected={(span) => addSelectedSpan(span)}
            onSpanSelected={(span) => handleAppendText(span)}
          />
        )}
      />
  );
